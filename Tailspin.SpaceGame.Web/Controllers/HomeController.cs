using System;
using System.Collections.Generic;
using System.Diagnostics;
using System.Linq;
using System.Linq.Expressions;
using System.Threading.Tasks;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using TailSpin.SpaceGame.Web.DB;
using TailSpin.SpaceGame.Web.Models;

namespace TailSpin.SpaceGame.Web.Controllers
{
    public class HomeController : Controller
    {
        private ProfileController ProfileController => new ProfileController(_dbContext);
        private ScoresController ScoresController => new ScoresController(_dbContext);
        private List<string> GameModes => new List<string>()
                {
                    "Solo",
                    "Duo",
                    "Trio"
                };

        private List<string> GameRegions => new List<string>()
                {
                    "Milky Way",
                    "Andromeda",
                    "Pinwheel",
                    "NGC 1300",
                    "Messier 82",
                };

        // // High score repository.
        // private readonly IDocumentDBRepository<Score> _scoreRepository;
        // // User profile repository.
        // private readonly IDocumentDBRepository<Profile> _profileRespository;
        private TailspinContext _dbContext;

        public HomeController(TailspinContext dbContext)
        {
            _dbContext = dbContext;
        }

        public async Task<IActionResult> Index(
            int page = 1,
            int pageSize = 10,
            string mode = "",
            string region = ""
            )
        {
            // Create the view model with initial values we already know.
            var vm = new LeaderboardViewModel
            {
                Page = page,
                PageSize = pageSize,
                SelectedMode = mode,
                SelectedRegion = region,
                GameModes = GameModes,
                GameRegions = GameRegions
            };

            try
            {
                // Fetch the scores that match the current filter.
                IEnumerable<Score> scores = await ScoresController.GetScoresAsync(mode, region, page - 1, pageSize);

                // Wait for the total count.
                vm.TotalResults = scores.Count();

                // Set previous and next hyperlinks.
                if (page > 1)
                {
                    vm.PrevLink = $"/?page={page - 1}&pageSize={pageSize}&mode={mode}&region={region}#leaderboard";
                }
                if (vm.TotalResults > page * pageSize)
                {
                    vm.NextLink = $"/?page={page + 1}&pageSize={pageSize}&mode={mode}&region={region}#leaderboard";
                }

                // Fetch the user profile for each score.
                // This creates a list that's parallel with the scores collection.
                var profiles = new List<Profile>();
                foreach (var score in scores)
                {
                    var profile = ProfileController.GetProfile(score.ProfileId);

                    if(profile != null)
                    {
                        profiles.Add(profile);
                    }
                }

                // Combine each score with its profile.
                vm.Scores = scores.Zip(profiles, (score, profile) => new ScoreProfile { Score = score, Profile = profile });

                return View(vm);
            }
            catch (Exception)
            {
                return View(vm);
            }
        }

        [Route("/profile/{id}")]
        public IActionResult Profile(int id, string rank = "")
        {
            try
            {
                // Fetch the user profile with the given identifier.
                return View(new ProfileViewModel { Profile = ProfileController.GetProfile(id), Rank = rank });
            }
            catch (Exception)
            {
                return RedirectToAction("/");
            }
        }

        public IActionResult Privacy()
        {
            return View();
        }

        [ResponseCache(Duration = 0, Location = ResponseCacheLocation.None, NoStore = true)]
        public IActionResult Error()
        {
            return View(new ErrorViewModel { RequestId = Activity.Current?.Id ?? HttpContext.TraceIdentifier });
        }
    }
}